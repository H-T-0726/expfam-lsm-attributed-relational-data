"""
Step 3.8 Experiment 2 -- RMSE vs d, all families, all parameters.

For each family (Bernoulli, Poisson, Gaussian) and each d in D_LIST,
runs N_TRIALS independent EM fits and records:
  RMSE(Z), RMSE(F), RMSE(Sigma), RMSE(Y), RMSE(X), |w0 err|, |w err|

Replicates and extends NOLTA 2024 Table IV to all 3 distribution families.

Settings: n=150, k=3, L=10, num_iter=10, n_trials=5

Output:
    expfam/results/synthetic_d_all_trials.csv
    expfam/results/synthetic_d_all_summary.csv
    expfam/results/synthetic_d_all_plot.png
"""
...
def main():
    print("=" * 65)
    print("  Step 3.8 Exp2 -- RMSE vs d, all families, all params")
    print(f"  d_list={D_LIST}, n={N}, k={K_TRUE}, L={L}, "
    ...
    for family in FAMILIES:
        ...
        for d in D_LIST:
            for trial in range(N_TRIALS):
                data_seed  = BASE_SEED + trial * 23
                model_seed = BASE_SEED + trial * 23 + 1
                ...
                rec = {"family": family, "d": d, "trial": trial}
                for m in METRICS:
                    rec[m] = res[m]
            ...
            vals = {m: [r[m] for r in records
                        if r["family"] == family and r["d"] == d]
                    for m in METRICS}
            print(f"  d={d:3d}  Z={np.mean(vals['rmse_Z']):.4f}  "
    ...
    trial_csv = OUTPUT_DIR / "synthetic_d_all_trials.csv"
    ...
    summary = df.groupby(["family", "d"]).agg(**agg_dict).reset_index()
    summary_csv = OUTPUT_DIR / "synthetic_d_all_summary.csv"
    ...
    # ----------------------------------------------------------------
    print("\n" + "=" * 80)
    print("  NOLTA Table IV equivalent: Poisson, RMSE vs d (mean over 5 trials)")
    print("=" * 80)
    print(f"{'d':>5}" + "".join(f"  {METRIC_LABELS[m]:>10}" for m in METRICS))
    print("-" * 80)
    ...
    for ax_i, m in enumerate(METRICS):
        ...
        for fam in FAMILIES:
            ...
            ax.errorbar(d_vals, means, yerr=stds,
        ...
        ax.set_title(f"{METRIC_LABELS[m]} vs d", fontsize=11)
    ...
    plt.suptitle(
        f"All-Parameter RMSE vs d -- 3 Families\n"
        f"(n={N}, k={K_TRUE}, L={L}, num_iter={NUM_ITER}, n_trials={N_TRIALS})",
        fontsize=12
    )
    plt.tight_layout()
    plot_path = OUTPUT_DIR / "synthetic_d_all_plot.png"
    ...
    print("\n[experiment_synthetic_d_all DONE]")
