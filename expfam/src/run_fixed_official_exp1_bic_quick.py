"""
fixed版 Exp1 BIC quick 実験スクリプト。

DualExpFamLSMFixed (Y-side 0.5 なし) を使って Exp1 (BIC k選択) を実行する。
3シナリオ × k=1~6 × 3試行 = 54 fits。

確認目的:
  - fixed版で calc_Q_dual_strict / calc_bic_dual が正しく使えるか
  - fixed版でも BIC で k=3 付近が選ばれるか
  - fixed 公式 runner の基本構造が正しく動くか

既存ファイルは一切変更しない。
出力先: expfam/results/fixed_official/quick/

実行例:
  cd expfam/src
  python run_fixed_official_exp1_bic_quick.py
"""
...
OUT_DIR = _ROOT / "expfam" / "results" / "fixed_official" / "quick"

N, D, K_TRUE = 150, 15, 3
L, NITER     = 5, 8
K_LIST       = [1, 2, 3, 4, 5, 6]
TRIALS       = [0, 1, 2]   # quick版: 3試行のみ
...
EXP1_BASE    = 4000   # exp_scenario_lib.run_exp1 default base_seed
...
def run_em_fixed_bic(X, Y, true_params, family_x, family_y, k,
    ...
    Computes Q_strict at the end for BIC.

    Based on run_mismatch_fixed.run_em_fixed, extended to return Q_strict.
    ...
    for retry in range(max_retries + 1):
        ...
        # Informed init: Y side (mirrors run_em_dual in utils_expfam.py)
    ...
    # Ensure model.params holds latest values before Q_strict computation
    # (M-step updates local vars but not model.params["sigma"] etc.)
    model.params.update({"F": F, "sigma": sigma, "w0": w0, "w": w})

    # Q_strict: DualExpFamLSMFixed is a subclass of DualExpFamLSM,
    # calc_log_likelihood_X / _Y are inherited and unchanged → works directly
...
def main():
    ...
    print(f"fixed Exp1 BIC quick: {len(SCENARIOS)} scenarios x "
        ...
        for k_est in K_LIST:
            for trial in TRIALS:
                ...
                rows.append({
                    ...
                    "note":          f"fixed_official quick k_true={K_TRUE}",
                })

    df = pd.DataFrame(rows)

    # ── summary CSV ────────────────────────────────────────────────────
    out_summary = OUT_DIR / "fixed_exp1_bic_quick_summary.csv"
    ...
    for (scen_tag, k_est), sub in df.groupby(
        ...
        agg_rows.append({
            ...
            "note":          "fixed_official quick",
        })
    agg_df = pd.DataFrame(agg_rows)
    out_agg = OUT_DIR / "fixed_exp1_bic_quick_agg.csv"
    ...
    for scen_tag, sub_agg in agg_df.groupby("scenario", sort=True):
        ...
        bestk_rows.append({
            ...
            "note": (
                f"fixed_official quick, BIC mean over {len(TRIALS)} trials"
            ),
        })
    ...
    out_bestk = OUT_DIR / "fixed_exp1_bic_quick_bestk.csv"
    ...
    print("\n=== num_params (KI-010 reference) ===")
    ref_df = agg_df[agg_df["scenario"] == "A"][["k_est", "bic_mean"]].copy()
    from utils_expfam import calc_bic_dual as _cbd
    for k in K_LIST:
        for scen_tag, fam_x, fam_y in SCENARIOS:
            _, np_val = _cbd(0.0, k, N, D, fam_x, fam_y)
            row = agg_df[
                (agg_df["scenario"] == scen_tag) & (agg_df["k_est"] == k)
            ]
            if len(row):
                print(
                    f"  Scen {scen_tag} k={k}: num_params={np_val}"
                    f"  BIC_mean={row['bic_mean'].iloc[0]:.1f}"
                )
