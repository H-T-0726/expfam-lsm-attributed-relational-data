...
## Phase 8b K_TRUE robustness full sweep — Attempt 2（2026-09-04、issue #59、objective-consistent prototype使用）

Phase 7e (Issue #43) で凍結した leakage-safe held-out K-selection protocol を、**generator の
`K_TRUE` を {1,2,4,5} へ拡張**して **exactly once** 実行した本番 sweep。人間が Issue #59
comment `5529711820` で承認した scope どおり **336 fits ちょうど**（A 168 / B 168）を実行し、
独立 artifact 監査（`--mode full`）で PASS を得た。`K_TRUE=3` は Phase 7e の 42 fits を
**READ-ONLY で参照するだけ**で再実行していない（`phase7e_rerun_count = 0`）。

**Attempt 1（`k_true_robustness_full_20260902/`）は ABORTED_BY_OPERATOR_INTERRUPT であり、
provenance 記録としてのみ保全する。科学的主張には使用しない。** Attempt 1 の部分結果は
Attempt 2 に一切再利用していない（`partial_results_reused = False`）。両ディレクトリを
削除・改変しない。

**K 選択基準の明示（混同防止）:** 本実験の K 選択は **frozen Phase 7e held-out predictive
score**（held-out Bernoulli raw-eta plug-in mean log score、2 start の非加重平均、
tie tolerance 1e-12、smallest-K tie rule）である。**`Q_strict` / EM の Q 関数基準 /
ICL-type complete-data criterion / Schwarz BIC / marginal likelihood は使用していない。**
`calc_bic_dual` に関する KI-010 の論点は legacy 基準の話であって本実験の基準とは別物である
（KI-019）。

| 実験ID | 内容 | 実装/スクリプト | 結果CSV | 図 | 状態 | 原稿採用 | 注意 |
|------|----|----------|-------|---|----|------|----|
| Phase 8b K_TRUE robustness full sweep（Attempt 2） | estimand A（primary、`w_true = 1.5` 固定）と estimand B（sensitivity、`w_K = 1.5·√(3/K_TRUE)`）の 2 系列について、`K_TRUE ∈ {1,2,4,5}` × replicate {1,2,3} × candidate K {1,...,7} × start {1,2} = **各 168 fits・合計 336 fits** を実行（family_x=poisson、family_y=bernoulli、n=75、d=15、L=5、num_iter=8、`numerics_mode="consistent"`、test_ratio=0.20、mask_design=S_C、random_design=CRN、hierarchy=H3_A、w0_true=-1.0）。seed base: data 51000 / model 530000 / anchor split 42000（split seed 42001-42003 は Phase 7e と意図的に共有し、`anchor_match=True` を 24 セルすべてで確認） | `tools/research_audit/run_k_true_robustness_sweep.py`（`--full --allow-em --confirm-k-true-sweep --estimand AB`）, `tools/research_audit/audit_k_true_robustness_sweep.py`（`--mode full --write-report`）, `tools/research_audit/build_k_true_robustness_report.py`, `expfam/src/experimental/model_dual_expfam_consistent.py` | `expfam/results/k_selection/k_true_robustness_full_attempt2_20260904/full_fit_results.csv`, 同ディレクトリ `selection_matrix.csv`, `full_summary.json`, `runinfo.json`, `authorization.json`, `manifest.csv`, `mask_provenance.csv`, `config_gate.csv`, `leakage_gate.csv`, `audit_report.json` | （なし） | current_support | ✗ | objective-consistent prototype（lineage E）; Issue #59（protocol origin は Issue #49）; report は `reports/k_selection_theory/k_true_robustness_full_report_20260904.md`。**provenance**: role 1 approved scientific baseline `68c78e1191889609dead05ea5a9fb11525ce92e2`、role 2 reviewed full-execution main `ddc9b0b4c38da995fedf43ceef12f17dfb4db353`、role 3 runtime RUN_CODE_SHA `ef85b4c921546129b8d4f7440f8a09a41aa652e5`、Phase 7e anchor RUN_CODE_SHA `b9311e64a7b36c0a8a9704fff0ee7b38efe36a8a`、frozen protocol hash `2d19c5fe6edadd0823925ed7dd051cb27837bccf51d5102e0bcee53271654eb9`、`execution_attempt_id = phase8b-full-attempt-2`、human approval Issue #59 comment `5529711820`。**実行数**: full 336（A 168 / B 168、global fit_index 1..336、重複 0 / 欠番 0）/ K_TRUE=3 新規行 0 / retry 0 / replacement 0 / seed rescue 0 / tolerance 緩和 0 / Phase7e rerun 0 / canary rerun 0 / smoke rerun 0。**結果（記述のみ、新規グリッド {1,2,4,5}）**: A の selected K は K_TRUE=1 で [1,1,1]、K_TRUE=2 で [2,3,2]、K_TRUE=4 で [4,4,4]、K_TRUE=5 で [4,4,5]（真値一致 **9/12**）。B は K_TRUE=1 で [1,1,1]、K_TRUE=2 で [2,3,2]、K_TRUE=4 で [4,4,4]、K_TRUE=5 で [3,4,5]（真値一致 **9/12**）。A と B が異なるのは K_TRUE=5 replicate 1 のみ（A=4 / B=3）。**`K_TRUE=3` は Phase 7e anchor の READ-ONLY 参照であり selected K = [3,3,5]・真値一致 2/3。A 行と B 行は同一の anchor 証拠を共有しており、6 個の独立実験ではない。** 全 24 セルで tie 候補は 1 個のみ。**integrity**: 336/336 clean、`finite_state` 全 True、`q_failure` 全 False、`nan_occurred` 全 False、`warning_count` 全 0、`internal_retry` 全 0、`failure.json` 不在、`config_gate` 103 件すべて passed、`leakage_gate` 336 行すべて pre/post passed かつ `fit_boundary_status=clean`、独立 full audit PASS（BLOCKER 0 / HIGH 0 / MEDIUM 0、findings 空）。**証拠の数え方**: 新規 336 + Phase 7e anchor 42（READ-ONLY 再利用）= **統合ユニーク 378**。**420 ではない**（A/B が同一 anchor を参照するため anchor を二重計上しない）。**限定**: ①各条件 3 replicate のみ。p 値・信頼区間・検出力・bootstrap は一切計算していない ②score は plug-in held-out mean log score であり posterior predictive / marginal likelihood / ELBO ではない。**Q_strict / ICL-type / Schwarz BIC は使用していない（KI-019）** ③transductive dyad holdout であり inductive 一般化は評価していない ④prototype・本文採用不可 ⑤consistency / asymptotic consistency / universal K recovery / BIC consistency / 理論保証は主張しない ⑥`K_TRUE=5` の under-selection の原因は未同定（margin を記述しただけ）。A/B 差 1 セルから信号強度スケーリングの一般効果を推論しない ⑦再実行・置換 fit・seed 変更・tolerance 緩和・retry はいずれも行っていない |
| Phase 8b K_TRUE robustness full sweep（Attempt 1、**中断・provenance のみ**） | 同一 protocol hash の full sweep を開始したが operator interrupt により fit_index 3 で停止。`attempted_fit_count = 3` / `clean_fit_calls = 2` / `scored_rows = 0` / `retry_count = 0` / `replacement_fits_executed = 0`、`status = FAILED` | `tools/research_audit/run_k_true_robustness_sweep.py`（RUN_CODE_SHA `1946953ffc7e7db586dda2933c9a25a6f0235d07`） | `expfam/results/k_selection/k_true_robustness_full_20260902/failure.json`, 同ディレクトリ `authorization.json`, `manifest.csv`, `mask_provenance.csv`, `config_gate.csv`, `leakage_gate.csv`, `full_fit_results.csv`（**header のみ・data 行 0**） | （なし） | **aborted** | ✗ | **ABORTED_BY_OPERATOR_INTERRUPT / provenance only / no scientific use。** `artifact_version = phase8b-full-artifact-v1`。この 2 clean fits を科学的主張の根拠にしない。Attempt 2 は本結果を再利用していない（`partial_results_reused = False`）。**削除しない・改変しない** |

---

## clean true-K n-sweep（2026-09-04、identifiability フェーズ、canonical clean generator 使用）

先生のゼミ指摘（「K_TRUE を設定しただけでは識別可能な真の K とは限らない」「`n→∞` の
理論的性質を考える必要がある」）に対応するため、**canonical model から literal に生成する
新しい generator** を作り、その上で `n` を掃引した K-selection 実験を exactly once 実行したもの。

**historical generator（`expfam/src/data_generator_expfam.py`）は使っていない。** 一次コードの
確認により、historical generator は `Z` と Gaussian-X を事後に列 z-score し、`F` の行を正規化し、
Poisson の `η` を `[−20,10]` で切り、`sigma_x_true` を使っていないことが判明している（KI-021）。
これらは **Poisson-X 識別可能性命題（unclipped link と iid `N(0,I)` を要求）の前提を壊す**ため、
本実験では forward-only の `data_generator_canonical.py`（`canonical-clean-v1`）を新設した。
**historical generator は 1 文字も変更していない。**

**`K_TRUE` と `K*` は別物である。** 本実験の「真値一致」は `K_TRUE`（generator の手続き上の設定）
との一致であって、観測分布の最小潜在次元 `K*` との一致ではない（`RESEARCH_MASTER.md` §17.2）。

**選択基準の名称:** S1 は held-out Bernoulli raw-eta plug-in mean log score（Phase 7e/8b と同一定義）。
S2 は `−2 Q_strict + p log n` の **Q ベース基準であり Schwarz BIC ではない**（KI-010・KI-019）。
S3 は本 runner 内で完結に定義した **plug-in conditional 基準であり、原論文 Eq.(26) ではない**
（原論文の評価手続きは本文から特定不能。`paper_bic_reproduction_alignment_20260904.md`）。
S4 は構造診断であり **selected K を作らない**。

| 実験ID | 内容 | 実装/スクリプト | 結果CSV | 図 | 状態 | 原稿採用 | 注意 |
|------|----|----------|-------|---|----|------|----|
| clean true-K n-sweep | canonical clean generator（`canonical-clean-v1`）で `K_TRUE ∈ {1,3,5}`（反復 4/4/8）× `n ∈ {50,75,100,150}` × candidate K {1..7} × start {1,2} = **896 fits** を実行（family_x=poisson、family_y=bernoulli、d=15、L=5、num_iter=8、`numerics_mode="consistent"`、test_ratio=0.20、w0_true=−1.0）。**信号強度を K_TRUE 間で整合**させるため `f_scale = sqrt(0.5·d/K)`（平均 `‖f_l‖²=0.5` 一定）と `w_K = 1.0·sqrt(3/K)`（`w²K=3.0` 一定）を採用。seed: data `810000+K·10000+n·10+rep`、split `820000+…`、model `830000+K·100000+n·1000+rep·100+K_est·10+start` | `tools/research_audit/run_clean_true_k_sweep.py`（`--full --allow-em --confirm-clean-true-k-sweep`）, `tools/research_audit/audit_clean_true_k_sweep.py`（`--write-report`）, `tools/research_audit/build_clean_true_k_report.py`, `expfam/src/experimental/data_generator_canonical.py`, `expfam/src/experimental/model_dual_expfam_consistent.py` | `expfam/results/k_selection/clean_true_k_asymptotics_20260904/fit_results.csv`, 同ディレクトリ `selection_matrix.csv`, `gram_spectrum.csv`, `generator_provenance.csv`, `manifest.csv`, `protocol.json`, `summary.json`, `runinfo.json`, `audit_report.json` | （なし） | current_support | ✗ | objective-consistent prototype（lineage E）; report は `reports/identifiability/clean_true_k_results_20260905.md`。**provenance**: protocol hash `547880a16aef6530cfdf7903c4e32f16062397e0bacc0c109d5c77fb9892ccc0`、run_code_sha `63e1202258a71256a55732fc1832db13d7f7b2bd`、generator `canonical-clean-v1`、python 3.13.14 / numpy 2.3.5、wall clock 8823.1 s。**実行数**: 896（expected=actual=unique、重複 0・欠番 0）/ retry 0 / replacement 0 / seed rescue 0 / tolerance 緩和 0 / resume False / NaN 0 / 非有限 0 / Q・BIC 失敗 0。**generator 不変量**（全 64 セル）: `rank(F)=K_TRUE`、`normalization_policy="none"`、`link_policy="canonical_no_clipping_fail_fast"`、平均 `‖f_l‖²=0.500000`、`w²K=3.000000`、Y density 0.318–0.340。**結果（真値一致、記述のみ）**: S1（held-out）K_TRUE=1 で 4/4・4/4・4/4・4/4、K_TRUE=3 で 1/4・1/4・3/4・4/4、K_TRUE=5 で 2/8・0/8・4/8・8/8（合計 **39/64**）。S2（Q ベース）K_TRUE=1 で 4/4 ×4、K_TRUE=3 で 2/4・3/4・4/4・4/4、K_TRUE=5 で 0/8・0/8・1/8・7/8（合計 **37/64**）。S3（plug-in conditional）合計 **3/64** でほぼ全セルが候補上限 K=7。K_TRUE=5 の平均 selected K は S1 が 2.62→3.00→4.50→5.00、S2 が 1.75→3.25→3.62→4.88。criterion 一致は S1 vs S2 44/64、S1 vs S3 2/64、S2 vs S3 0/64、三者一致 0/64。S4 は全 64 セルで推定 Gram の最小固有値が負（中央値 −1.80〜−0.52）、閾値なし階数は常に d=15 で **K を返さない**。**独立監査**: artifact-only auditor（runner を import せず全定数を独立 literal 保持、selection を生値から再計算）verdict **PASS**、BLOCKER 0 / HIGH 0 / MEDIUM 0 / LOW 2。**限定**: ①**有限標本の記述にすぎない**。`n` は 4 点、反復は 4 または 8。信頼区間・検定・検出力は一切計算していない ②**平均 selected K は単調増加したが真値一致数は単調でない**（S1 は n=75 で 0/8 に下がる）。**「n を増やすと K_TRUE に収束した」とは書かない** ③**`K_TRUE=1` の 4/4 は good recovery の証拠ではない**。支配的な誤り方が under-selection で、`K=1` は候補下端、tie rule は最小 K を選ぶため**下限効果と交絡**している ④**一致は `K_TRUE` との一致であって `K*` との一致ではない**（U9 未検証）⑤`family_y=bernoulli` は識別可能性（U2）も非入れ子性（U5）も**未証明の領域**。強い理論結果 P2・P3 は Gaussian-Y 限定 ⑥**S1 が population で何を選んでいるかは未解決（U10）**。plug-in raw-eta score は proper scoring rule ではない ⑦**S3 の失敗を原論文 BIC の失敗と読んではならない** ⑧**有効標本数が未定義**。S2・S3 の罰則は `log n`（ノード数）を使うが Y は `n(n−1)/2` dyad を供給する ⑨prototype・**本文採用不可** ⑩再実行・置換 fit・seed 変更・tolerance 緩和・retry・resume はいずれも行っていない |
