...
B.append(tbl([
    ["節","主題","ポイント"],
    ["2.1","提案の核心（3行）","X×Y を ExpFam に一般化；posterior は一般に closed form でない"],
    ["2.2","生成モデル","z_i ∼ N(0,σ_z²I), x_ij ∼ ExpFam_X, y_ij ∼ ExpFam_Y の定義"],
    ["2.3","complete-data log-likelihood","prior / X / Y の3項分解"],
    ["2.4","なぜ MCEM か","E-step の期待値が解析計算不能 → モンテカルロ近似"],
    ["2.5","なぜ Laplace 近似か","posterior が非 Gaussian → MAP 近傍の2次近似"],
    ["2.6","一般形の導出","gradient → Hessian → A''(η) が自然に現れる"],
    ["2.7","シナリオ A/B/C の具体化","3分布族での A''(η) の形と精度行列"],
    ["2.8","後退互換性","Gauss×Bern → Mikawa 2024, Gauss×Gauss → 2022 型"],
    ["トグル","先行研究詳細","Mikawa 2022 / 2024 の数式導出（参照用）"],
]))
B.append(h2("2.1 提案の核心を先に述べる"))
B.append(callout(
    "Dual-ExpFam LSM の核心は3点。\n\n"
    "① 属性データ X と関係データ Y をともに任意の指数型分布族でモデル化する。\n\n"
    "② 推論の主対象は潜在変数 z_i の事後分布 p(z_i | X, Y, θ) だが、\n"
    "   Y が非 Gaussian 分布族のとき事後分布は closed form（解析解）を持たない。\n\n"
    "③ そこで Laplace 近似（事後分布を Gaussian で近似）と\n"
    "   MCEM（期待値をサンプル平均で近似）を組み合わせて推論する。\n\n"
    "精度行列の変更点は1式のみ：\n"
    "   Bernoulli 固有の s_ij(1−s_ij)  →  分散関数 A_Y''(η_ij^Y)\n"
    "この置換により任意の指数型分布族に対応できる。",
    "🎯", "yellow_background"
))
B.append(para(rt(
    "先行研究との関係：Mikawa 2022 は Y = Gaussian なので事後分布が解析的 Gaussian に落ちる（MCEM 不要）。"
    "Mikawa 2024 は Y = Bernoulli に拡張し、Laplace 近似 + MCEM を導入した。"
...
B.append(h3("2.2.1 指数型分布族の定義"))
B.append(para(rt(
    "指数型分布族とは、密度関数（確率質量関数）が以下の形に書けるクラスである："
)))
...
B.append(callout(
    "重要性質：A'(η) = E[T(Y)]（平均）、A''(η) = Var[T(Y)]（分散）。\n"
    "A''(η) はのちに Laplace 近似の曲率（精度行列）として Hessian から自然に現れる。",
    "💡", "blue_background"
))
B.append(h3("2.2.2 本提案の生成モデル"))
B.append(para(rt(
    "n ノード、潜在次元 k、属性次元 d として："
)))
B.append(eq_block(r"z_i \sim \mathcal{N}(0,\,\sigma_z^2 I_k), \quad i = 1,\ldots,n"))
B.append(para(rt("属性データ（第 j 特性量）：")))
B.append(eq_block(
    r"p(x_{ij}\mid z_i) = h_X(x_{ij})\exp\!\bigl(\eta_{ij}^X\,T_X(x_{ij}) - A_X(\eta_{ij}^X)\bigr),"
    r"\quad \eta_{ij}^X = f_j^\top z_i"
))
B.append(para(rt("関係データ（i < j の対）：")))
B.append(eq_block(
    r"p(y_{ij}\mid z_i,z_j) = h_Y(y_{ij})\exp\!\bigl(\eta_{ij}^Y\,T_Y(y_{ij}) - A_Y(\eta_{ij}^Y)\bigr),"
    r"\quad \eta_{ij}^Y = w_0 + w\,z_i^\top z_j"
))
B.append(tbl([
    ["パラメータ","記号","備考"],
    ["潜在変数","z_i ∈ R^k","ノード i の潜在座標（推論対象）"],
    ["因子行列","F ∈ R^{d×k}（行 f_j ∈ R^k）","X の負荷行列（M-step で更新）"],
    ["関係パラメータ","w_0 ∈ R, w ∈ R","バイアスと内積スケール"],
    ["潜在空間スケール","σ_z² > 0","事前分布の分散"],
    ["分散パラメータ","φ_X, φ_Y","Gaussian のとき φ = σ²、非 Gaussian のとき φ = 1"],
]))
B.append(callout(
    "φ（分散パラメータ）の役割：\n"
    "Bernoulli・Poisson は自然母数だけで分布が完全に定まる（φ = 1）。\n"
    "Gaussian の場合は分散 σ² が追加パラメータとして現れ、φ = σ² となる。\n"
    "φ を明示することで Hessian の係数が統一的に (1/φ) × A''(η) と書ける。",
    "💡", "blue_background"
))
...
B.append(para(rt(
    "Z を観測済みと仮定したときの完全データ対数尤度を prior / X / Y の3項に分解する："
)))
B.append(eq_block(
    r"\ln p(X,Y,Z\mid\theta) = "
    r"\underbrace{-\frac{1}{2\sigma_z^2}\sum_i\|z_i\|^2+\cdots}_{\text{prior}}"
    r"+ \underbrace{\sum_{i,j}\bigl[\eta_{ij}^X T_X(x_{ij})-A_X(\eta_{ij}^X)\bigr]}_{X\text{ term}}"
    r"+ \underbrace{\sum_{i<j}\bigl[\eta_{ij}^Y T_Y(y_{ij})-A_Y(\eta_{ij}^Y)\bigr]}_{Y\text{ term}}"
))
B.append(para(rt(
    "Z は潜在変数であるため観測されない。EM アルゴリズムでは Q 関数（完全データ対数尤度の事後期待値）を"
    "最大化することで間接的に θ を推定する。次節でこの期待値がなぜ厳密計算できないかを示す。"
)))
...
B.append(para(rt(
    "EM アルゴリズムは完全データ対数尤度の条件付き期待値（Q 関数）を最大化する："
)))
B.append(eq_block(
...
B.append(para(rt(
    "Q 関数を評価するには事後分布 p(Z|X,Y,θ) に関する積分が必要："
)))
B.append(eq_block(
    r"Q = \int \ln p(X,Y,Z\mid\theta)\,p(Z\mid X,Y,\theta^{\mathrm{old}})\,dZ"
))
B.append(para(rt(
    "この積分が解析的に計算できるのは、事後分布が既知の形（例：Gaussian）になる場合だけである。"
    "しかし Y が Bernoulli や Poisson のとき事後分布は Gaussian にならない（2.5 節で詳述）。"
    "したがってこの積分を厳密に評価する方法がない。"
)))
B.append(callout(
    "直感：本来は posterior に関する期待値を厳密に積分したい。\n"
    "しかし p(z_i | X, Y) が複雑な非 Gaussian 分布であるため\n"
    "∫ f(z_i) p(z_i | X, Y) dz_i を解析的に計算できない。\n"
    "そこで posterior から L 個のサンプルを引いてその平均で近似する。\n"
    "これが Monte Carlo EM（MCEM）である。",
    "💡", "blue_background"
))
B.append(h3("2.4.3 MCEM による Q の近似"))
B.append(para(rt(
    "Laplace 近似（2.5 節）で近似事後分布 q(Z) を得たあと、"
    "q(Z) から L 個のサンプル z_i^{(1)}, ..., z_i^{(L)} を引き Q を近似する："
)))
B.append(eq_block(
...
B.append(para(rt(
    "L → ∞ のとき MCEM の Q は真の Q に確率収束する。本実装では L=5 を採用。"
)))
...
B.append(para(rt(
    "z_i に関する条件付き対数事後分布（定数を除く）："
)))
B.append(eq_block(
    r"\ln p(z_i\mid\cdots) = "
    r"-\frac{\|z_i\|^2}{2\sigma_z^2}"
    r"+ \sum_{j=1}^d\bigl[f_j^\top z_i\cdot T_X(x_{ij}) - A_X(f_j^\top z_i)\bigr]"
    r"+ \sum_{j\neq i}\bigl[\eta_{ij}^Y T_Y(y_{ij}) - A_Y(\eta_{ij}^Y)\bigr]"
    r"+ \mathrm{const}"
))
B.append(h3("2.5.2 なぜ Gaussian posterior が閉じないか"))
B.append(para(rt(
    "事前分布 p(z_i) ∝ exp(−||z_i||²/2σ_z²) は z_i の二次形式 → Gaussian 構造を持つ。"
    "しかし X 側と Y 側の対数尤度は A_X(f_j^T z_i) や A_Y(w_0+w z_i^T z_j) という非線形関数を含む："
)))
B.append(tbl([
    ["分布族","A(η)","A(f_j^T z_i) の z_i 依存性","posterior の形状"],
    ["Gaussian","η²/2","(f_j^T z_i)² / 2  — 二次形式","二次形式 → Gaussian ✓"],
    ["Bernoulli","ln(1+e^η)","ln(1+exp(f_j^T z_i))  — 非二次","非 Gaussian ✗"],
    ["Poisson","e^η","exp(f_j^T z_i)  — 非二次","非 Gaussian ✗"],
]))
B.append(callout(
    "直感：posterior 全体は複雑だが、最も確からしい点（MAP）の近くでは\n"
    "山の形を二次式で近似できる。その二次近似が Gaussian に対応する。\n"
    "これが Laplace 近似である。",
    "💡", "blue_background"
))
B.append(h3("2.5.3 Laplace 近似の手順"))
B.append(para(rt("STEP 1：MAP 推定値 z_i* を Newton 法で求める：")))
B.append(eq_block(r"z_i^* = \arg\max_{z_i}\ln p(z_i\mid X,Y,\theta)"))
B.append(para(rt("STEP 2：z_i* の近傍で対数事後分布を二次展開する。精度行列 Λ_i = −∇²_{z_i} ln p(...) を用いて：")))
B.append(eq_block(
    r"\ln p(z_i\mid\cdots) \approx "
    r"\mathrm{const} - \frac{1}{2}(z_i - z_i^*)^\top \Lambda_i (z_i - z_i^*)"
))
B.append(para(rt("STEP 3：これは z_i ∼ N(z_i*, Λ_i^{-1}) という Gaussian と一致する：")))
B.append(eq_block(r"q(z_i) = \mathcal{N}(z_i^*,\,\Lambda_i^{-1})"))
B.append(para(rt(
    "精度行列 Λ_i の具体的な形（A''(η) を含む）は次節 2.6 で Hessian から導出する。"
)))
...
B.append(callout(
    "この節が本文の最重要部分。\n"
    "gradient → Hessian の順に導出し、Hessian から A''(η) が自然に現れることを示す。",
    "📌", "gray_background"
))
...
B.append(eq_block(
    ...
    r"-\frac{\|z_i\|^2}{2\sigma_z^2}"
    r"+ \sum_{j=1}^d \bigl[f_j^\top z_i\cdot T_X(x_{ij}) - A_X(f_j^\top z_i)\bigr]"
    r"+ \sum_{j\neq i}\bigl[(w_0+w z_i^\top z_j) T_Y(y_{ij}) - A_Y(w_0+w z_i^\top z_j)\bigr]"
    r"+ \mathrm{const}"
))
...
B.append(para(rt("X 項（chain rule: ∂(f_j^T z_i)/∂z_i = f_j）：")))
B.append(eq_block(
    r"\nabla_{z_i}^X "
    r"= \sum_{j=1}^d \bigl[T_X(x_{ij}) - A_X'(f_j^\top z_i)\bigr]f_j"
    r"= \frac{1}{\varphi_X}F^\top\!\bigl[T_X(x_i) - A_X'(Fz_i)\bigr]"
))
B.append(para(rt("Y 項（chain rule: ∂(w_0 + w z_i^T z_j)/∂z_i = w z_j；対称補正 1/2 あり）：")))
B.append(eq_block(
    r"\nabla_{z_i}^Y "
    r"= \frac{w}{2\varphi_Y}\sum_{j\neq i}\bigl[T_Y(y_{ij}) - A_Y'(\eta_{ij}^Y)\bigr]z_j"
))
...
B.append(h3("2.6.3 Hessian と精度行列 — A''(η) の自然な登場"))
B.append(para(rt("事前分布項の Hessian（定数行列）：")))
B.append(eq_block(r"-\nabla^2_{z_i}[\text{prior}] = \frac{1}{\sigma_z^2}I"))
B.append(para(rt("X 項の Hessian（chain rule をもう1回：∂A_X'(f_j^T z_i)/∂z_i = A_X''(f_j^T z_i) f_j）：")))
B.append(eq_block(
    r"-\nabla^2_{z_i}[X\text{ term}]"
    r"= \sum_{j=1}^d A_X''(f_j^\top z_i)\,f_jf_j^\top"
    r"= \frac{1}{\varphi_X}F^\top\mathrm{diag}\!\bigl[A_X''(Fz_i)\bigr]F"
))
B.append(callout(
    "ここで A_X''(η) が Hessian から自然に出てくる。\n"
    "A_X''(η) = Var[T_X(X)|η] は η における「予測の不確かさ」（曲率）を表す。\n"
    "曲率が大きいほど精度行列への寄与が大きくなる。\n\n"
    "Bernoulli: σ(η)(1−σ(η)) ≤ 0.25、Poisson: e^η、Gaussian: 1（定数）",
    "💡", "blue_background"
))
B.append(para(rt("Y 項の Hessian（chain rule: ∂A_Y'(η)/∂z_i = A_Y''(η) · w z_j）：")))
B.append(eq_block(
    r"-\nabla^2_{z_i}[Y\text{ term}]"
...
B.append(eq_block(
    ...
    r"+ \underbrace{\frac{1}{\varphi_X}F^\top\mathrm{diag}\!\bigl[A_X''(Fz_i)\bigr]F}_{X\text{ term}}"
    r"+ \underbrace{\frac{w^2}{2\varphi_Y}\sum_{j\neq i}A_Y''(\eta_{ij}^Y)\,z_jz_j^\top}_{Y\text{ term}}"
))
B.append(callout(
    "Λ_i は正定値行列（A''(η) ≥ 0 の性質による）。\n"
    "3項はそれぞれ：事前分布の等方的正則化 + X の情報 + Y の情報。\n"
    "A''(η) が大きい → その点での曲率が大きい → 精度（確信度）が高い。",
    "💡", "blue_background"
))

B.append(h3("2.6.5 Laplace 近似事後分布"))
B.append(eq_block(
    r"q(z_i) = \mathcal{N}\!\bigl(z_i^*,\,\Lambda_i^{-1}\bigr)"
    r",\quad z_i^* = \arg\max\ln p(z_i\mid X,Y,\theta)"
))

B.append(h3("2.6.6 MCEM による Q 近似"))
B.append(para(rt("q(Z) から L サンプルを引き Q を近似する：")))
B.append(eq_block(
    r"\hat{Q} = \frac{1}{L}\sum_{l=1}^L\!\left["
    r"-\frac{\|z_i^{(l)}\|^2}{2\sigma_z^2}"
    r"+ \sum_{i,j}\bigl[\eta_{ij}^{X(l)} T_X(x_{ij}) - A_X(\eta_{ij}^{X(l)})\bigr]"
    r"+ \sum_{i<j}\bigl[\eta_{ij}^{Y(l)} T_Y(y_{ij}) - A_Y(\eta_{ij}^{Y(l)})\bigr]"
    r"\right]"
))

B.append(h3("2.6.7 M-step の更新方針"))
B.append(tbl([
    ["パラメータ","更新方法","適用条件"],
    ["F（因子行列）","解析解（最小二乗）","Gaussian X"],
    ["F（因子行列）","Adam 勾配法","Bernoulli / Poisson X"],
    ["σ_X²（分散）","閉形式 MLE","Gaussian X のみ"],
    ["w_0, w","Adam 勾配法","全分布族"],
    ["σ_Y²（分散）","閉形式 MLE","Gaussian Y のみ"],
]))

# ─── 2.7 シナリオ A/B/C への特殊化 ────────────────────────────────
B.append(h2("2.7 シナリオ A/B/C への特殊化"))
B.append(para(rt(
    "一般形に各分布族の A'(η), A''(η) を代入して、3シナリオの精度行列と gradient を求める。"
)))
B.append(tbl([
    ["分布族","A(η)","A'(η) = E[T(Y)]","A''(η) = Var[T(Y)]"],
    ["Bernoulli","ln(1+e^η)","σ(η) = 1/(1+e^{−η})","σ(η)(1−σ(η))"],
    ["Poisson","e^η","e^η","e^η"],
    ["Gaussian","η²/2","η（恒等写像）","1（定数）"],
]))

B.append(h3("2.7.1 Scenario A：Poisson-X × Bernoulli-Y"))
B.append(callout("Scenario A = カウント属性 × 2値ネットワーク（例：購買数 × 友人関係）。", "📋", "gray_background"))
B.append(para(rt("生成モデル：")))
B.append(eq_block(
    r"x_{ij}\sim\mathrm{Pois}(e^{f_j^\top z_i}),\quad"
    r"y_{ij}\sim\mathrm{Bern}\!\bigl(\sigma(w_0+w z_i^\top z_j)\bigr)"
))
B.append(para(rt("A_X''（Poisson）・A_Y''（Bernoulli）を代入：")))
B.append(eq_block(
    r"A_X''(f_j^\top z_i)=e^{f_j^\top z_i}"
    r"\;\Rightarrow\;\mathrm{diag}[A_X''(Fz_i)]=\mathrm{diag}(e^{Fz_i})"
    r",\quad A_Y''(\eta_{ij}^Y)=s_{ij}(1-s_{ij})"
))
...
B.append(h3("2.7.2 Scenario B：Gaussian-X × Poisson-Y"))
B.append(callout("Scenario B = 連続属性 × カウントネットワーク（例：特徴量 × 共著回数）。", "📋", "gray_background"))
B.append(para(rt("生成モデル：")))
B.append(eq_block(
    r"x_{ij}\sim\mathcal{N}(f_j^\top z_i,\,\sigma_X^2),\quad"
    r"y_{ij}\sim\mathrm{Pois}(e^{w_0+w z_i^\top z_j})"
))
B.append(para(rt("A_X''（Gaussian, φ_X=σ_X²）・A_Y''（Poisson, φ_Y=1）を代入：")))
B.append(eq_block(
    r"A_X''=1,\;\varphi_X=\sigma_X^2"
    r"\;\Rightarrow\;\frac{1}{\varphi_X}F^\top\mathrm{diag}[A_X'']F=\frac{1}{\sigma_X^2}F^\top F"
    r",\quad A_Y''(\eta_{ij}^Y)=\lambda_{ij}=e^{\eta_{ij}^Y}"
))
B.append(para(rt("精度行列：")))
B.append(eq_block(
...
B.append(eq_block(
    ...
    r"+ \frac{1}{\sigma_X^2}F^\top(x_i - Fz_i)"
    r"+ \frac{w}{2}\sum_{j\neq i}(y_{ij}-\lambda_{ij})\,z_j"
))

B.append(h3("2.7.3 Scenario C：Bernoulli-X × Gaussian-Y"))
B.append(callout("Scenario C = 2値属性 × 連続ネットワーク（例：バイナリ特徴 × 類似度行列）。", "📋", "gray_background"))
B.append(para(rt("生成モデル：")))
B.append(eq_block(
    r"x_{ij}\sim\mathrm{Bern}\!\bigl(\sigma(f_j^\top z_i)\bigr),\quad"
    r"y_{ij}\sim\mathcal{N}(w_0+w z_i^\top z_j,\,\sigma_Y^2)"
))
B.append(para(rt("A_X''（Bernoulli, φ_X=1）・A_Y''（Gaussian, φ_Y=σ_Y²）を代入：")))
B.append(eq_block(
    r"A_X''(f_j^\top z_i)=p_{ij}(1-p_{ij})\le 0.25"
    r",\quad A_Y''=1,\;\varphi_Y=\sigma_Y^2"
    r"\;\Rightarrow\;\frac{w^2}{2\varphi_Y}\sum_{j\neq i}z_jz_j^\top=\frac{w^2}{2\sigma_Y^2}\sum_{j\neq i}z_jz_j^\top"
))
B.append(para(rt("精度行列：")))
B.append(eq_block(
...
B.append(callout(
    "Scenario C の特徴：Y=Gaussian の A_Y''=1（定数）は n−1 個の j に対して加算され O(n) にスケール。\n"
    "X=Bernoulli の A_X'' ≤ 0.25 は d 次元で加算されても O(0.25 d) に留まる。\n"
    "n=150 ≫ d=15 の設定では Y 項が X 項を圧倒し、d を増やしても Z の推定精度に寄与しない。\n"
...
# ─── 2.8 後退互換性 ────────────────────────────────────────────
B.append(h2("2.8 後退互換性（先行研究との整合性確認）"))
B.append(para(rt(
    "Dual-ExpFam の一般形に特定の分布族を代入すると先行研究に正確に帰着することを確認する。"
)))
B.append(tbl([
    ["代入","Λ_i の X 側","Λ_i の Y 側","帰着先"],
    ["Gauss-X（φ_X=σ_X²）× Bern-Y（φ_Y=1）",
     "(1/σ_X²) F^T F",
     "(w²/2) Σ s_ij(1−s_ij) z_j z_j^T",
     "Mikawa 2024 Eq.(22) と一致"],
    ["Gauss-X（φ_X=σ_X²）× Gauss-Y（φ_Y=σ_Y²）",
     "(1/σ_X²) F^T F",
     "(w²/2σ_Y²) Σ z_j z_j^T",
     "Mikawa 2022 型の連続関係モデルに帰着"],
]))
B.append(callout(
    "後退互換の検証（Gauss-X × Bern-Y の場合）：\n"
    "A_X''=1 → (1/φ_X) F^T diag[1,...,1] F = (1/σ_X²) F^T F。\n"
    "A_Y''=s_ij(1-s_ij) → (w²/2) Σ s_ij(1-s_ij) z_j z_j^T（φ_Y=1 なので係数は w²/2）。\n"
    "これは Mikawa 2024 Eq.(22) の精度行列と完全に一致する。",
    "✅", "green_background"
))

# ─── トグル：先行研究の詳細 ────────────────────────────────────
B.append(toggle("▶ 先行研究詳細 (1)：Mikawa 2022 — 連続値関係モデルの導出", [
    callout(
        "注意：Mikawa 2022 の原論文は今回の一次資料として手元にない。\n"
        "以下は Mikawa 2024 本文で説明される従来モデルの設定と\n"
        "一般的な Gaussian 線形モデルの標準的な結果に基づく記述である。\n"
        "Mikawa 2022 の特定の式番号は引用しない。",
        "⚠️", "yellow_background"
    ),
    h3("生成モデル"),
    eq_block(
        r"z_i\sim\mathcal{N}(0,\sigma_z^2 I),"
        r"\quad x_{ij}\sim\mathcal{N}(f_j^\top z_i,\sigma_X^2),"
        r"\quad y_{ij}=z_i^\top z_j+e_{ij},\quad e_{ij}\sim\mathcal{N}(0,\sigma_Y^2)"
    ),
    para(rt("Y の線形モデル y_ij = z_i^T z_j + e_ij は w_0, w なしの単純内積モデル。")),
    h3("なぜ posterior が Gaussian か"),
    para(rt("Y = Gaussian なので A_Y''(η) = 1（定数）となり Hessian が定数行列になる：")),
    eq_block(
        r"\Lambda_i^{[2022]} = \frac{1}{\sigma_z^2}I"
        r"+ \frac{1}{\sigma_X^2}F^\top F"
        r"+ \frac{1}{\sigma_Y^2}\sum_{j\neq i}z_jz_j^\top"
    ),
    para(rt(
        "Λ_i が z_i に依存しない（A_Y''=1 定数のため）ので、勾配をゼロとおくと閉形式の事後平均が得られる。"
        "A_i（2022 の記法）= 共分散行列 = Λ_i^{-1}。"
    )),
    callout(
        "記法注意：Mikawa 2022 では A_i が共分散行列（= Λ_i^{-1}）を指す。\n"
        "Mikawa 2024 / Dual-ExpFam では Λ_i が精度行列を指す。\n"
        "1/2 係数：2022 は全ペア（j≠i）の和 → 係数 1/σ_Y²（1/2 なし）。\n"
        "2024/Dual は対称ペア i<j → 各ノードへの寄与が 1/2 → w²/2 が現れる。",
        "⚠️", "yellow_background"
    ),
    h3("E-step（閉形式）"),
    eq_block(r"q(z_i) = \mathcal{N}(\mu_i,\,A_i),\quad A_i=\Lambda_i^{-1},\quad \mu_i = A_i\cdot g_i"),
    h3("M-step（全パラメータが閉形式 MLE）"),
    tbl([
        ["パラメータ","更新式"],
        ["F","最小二乗解（閉形式 MLE）"],
        ["σ_X²","閉形式 MLE"],
...
B.append(toggle("▶ 先行研究詳細 (2)：Mikawa 2024 — Bernoulli 化と Laplace 近似の導入", [
    h3("生成モデル（2022 からの変更点）"),
    eq_block(
        r"y_{ij}\sim\mathrm{Bern}\!\bigl(\sigma(w_0+w z_i^\top z_j)\bigr)"
    ),
    para(rt("2022 の連続 y_ij を Bernoulli に置き換え、w_0, w を新たに導入。")),
    h3("事後分布の定義（Eq.(19)）"),
    eq_block(
        r"p(z_i\mid X,Y,\theta)\propto p(z_i)\prod_{j=1}^d p(x_{ij}\mid z_i)\prod_{j\neq i}p(y_{ij}\mid z_i,z_j)"
    ),
    h3("条件付き対数事後分布の分子（Eq.(20)）"),
    eq_block(
        r"\ln p(z_i\mid\cdots)\supseteq"
        r"-\frac{\|z_i\|^2}{2\sigma_z^2}"
        r"-\frac{1}{2\sigma_X^2}\sum_j(x_{ij}-f_j^\top z_i)^2"
        r"+\sum_{j\neq i}\bigl[(w_0+wz_i^\top z_j)y_{ij}-\ln(1+e^{w_0+wz_i^\top z_j})\bigr]"
    ),
    h3("Laplace 近似（Eq.(21)）"),
    eq_block(r"q(z_i)=\mathcal{N}(z_i^*,\Lambda_i^{-1})"),
    h3("精度行列（Eq.(22)）"),
    eq_block(
    ...
    h3("w_0, w の更新（Eq.(24)(25)）"),
    eq_block(
        r"\frac{\partial\hat{Q}}{\partial w_0}=\sum_{i<j}(y_{ij}-s_{ij}),\quad"
        r"\frac{\partial\hat{Q}}{\partial w}=\sum_{i<j}(y_{ij}-s_{ij})\,z_i^\top z_j"
    ),
    callout(
        "Mikawa 2024 の核心：Bernoulli の s_ij(1-s_ij) が精度行列の Y 側に現れる。\n"
        "本提案はこれを A_Y''(η) に置き換え、任意の指数型分布族に一般化した。",
        "💡", "blue_background"
    ),
]))
B.append(divider())
