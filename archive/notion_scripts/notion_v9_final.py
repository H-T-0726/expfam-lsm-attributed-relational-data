"""
Dual-ExpFam LSM — Notion v9: 教授報告用 最終極致版
Mikawa 2024 からの進化の完全可視化 + 全実験網羅 + 格調高い日本語
"""
...
B.append(toggle("▶ 数式の1対1対応（精度行列 Lambda_i）— 詳細展開", [
    ...
    eq_block(r"A''(\eta) \geq 0 \quad \text{（指数型分布族の普遍的性質 — 任意の分布で保証）}"),
...
B.append(eq_block(
    ...
    r"\underbrace{\bigl[T_Y(y_{ij}) - A_Y'(\eta_{ij}^{Y(l)})\bigr]}_{\text{分布族によって内容が変わる残差}}"
))
B.append(tbl([
