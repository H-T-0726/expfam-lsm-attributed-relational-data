...
> **（2026-07-19 更新注記）** 上 2 行は過剰な断定であり、root `CLAUDE.md` の
> 確定表現に置き換える。正確には：勾配・精度行列の両方に同じ 1/2 があることから
> 言えるのは「Y 側対数尤度を 1/2 倍した目的関数に対して自己整合的な
> Newton/Laplace になっている」ことまでである。真の（1/2 なしの）事後に対しては
> Newton 方向・mode・分散とも一般にずれ、分散の増大も「Y 側が支配的な方向で
> 最大約 2 倍まで大きくなり得る」のであって一律 2 倍ではない
> （Term1・Term2 には 1/2 が掛かっていないため）。
...
## 6. 2026-07-19 追記：結論の根拠の置き方（理論監査 P0-6）

本文書は 2026-05-08 時点で既に §2-A（calcAi 対角除去のバグ）・§2-B（calcGrad の
w 乗算欠如）を記録しており、これらは理論監査（2026-07-18/19）で再確認された。
この 2 点から、**MATLAB 実装は単独のゴールドスタンダードとして扱えない**。

したがって「1/2 は不要」という結論（変更なし）の根拠は、次の順に置く:

1. **主根拠**: 対称関係行列に対する独立した数式導出。
   Q_Y = Σ_{i<j} ℓ(y_ij, η_ij) を z_i で偏微分すると、i を含むペアが
   (i,j) [j>i] と (j,i) [j<i] の両側から各 1 回ずつ寄与して
   Σ_{j≠i} が現れ、1/2 は生じない（(1/2)Σ_{i≠j} 表記から出発しても同じ）。
2. **補助**: MATLAB calcAi に 1/2 がないという実装比較
   （ただし同ファイルに上記 2 点の未確認箇所があるため補助扱い）。

また §4 末尾の「ニュートン更新の方向は正しい」「分散が 2 倍に膨らむ」という
記述は過剰断定であり、§4 内の 2026-07-19 更新注記のとおり限定する。
先生への説明文面は `docs/teacher/half_factor_teacher_reply.md` の
「2026-07-19 追記」の限定表現を用いること。
