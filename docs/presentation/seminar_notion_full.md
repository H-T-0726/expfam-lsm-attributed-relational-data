...
$$x_{il} \mid z_i \sim \mathrm{ExpFam}_X(\eta_{il}^X), \quad \eta_{il}^X = f_l^\top z_i \quad \text{（属性データ。バイアスなし）}$$
...
| Term2：X 側の残差項 | X 尤度 | X の観測値と期待値（$A_X'(\eta)$）の残差を $F$ で潜在空間へ戻す。Gaussian-X では分散の逆数を考慮。詳細は `_calc_gradient` を参照 |
...
>
> **正しい 1/2（誤解しないこと）：**
> - Q 関数 Y 側の `0.5 * sum(ln_p)` → **正しい**（$\sum_{i<j}$ を全対称和で書くための正規化）
> - M-step 勾配 w0/w の `/2L` → **正しい**（$\sum_{i \neq j}$ を $\sum_{i<j}$ に換算するため）
>
> **Newton 方向への影響：** 0.5 が掛かっているのは Y 側 Term3 のみ。**Newton 方向が原稿採用式と完全に一致するとは断定できない**。修正は修論フェーズで対応予定。
>
...
| BIC | $-2\hat{Q}_{\mathrm{strict}} + \mathrm{num\_params} \cdot \ln n$。$k$ の選択に使用 |
