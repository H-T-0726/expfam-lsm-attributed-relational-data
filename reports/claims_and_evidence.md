...
> ⚠ **FROZEN — historical snapshot（2026-05-31 時点）。current canonical source ではない。**
>
> 本ファイルは 2026-05-31 に作成された当時の対応表であり、その後の実験フェーズ・監査を反映していない。
> **記載内容を現在の状態として引用しないこと。** 以下は当時の記録として保持するものであり、
> 事実関係が更新された箇所も本文を書き換えずそのまま残している（2026-07-19 の更新注記を除く）。
>
> **現在の正本:**
> - 数値の provenance（実験 → CSV → 図 → 主張）— `EXPERIMENT_REGISTRY.md`
> - 何を主張してよいか／まだ主張してはいけないか — `KNOWN_ISSUES.md`
> - 研究内容と主張の安全レベル — `RESEARCH_MASTER.md` §11
>
> **本ファイルの記述のうち、既に解消済みと判明しているもの（本文は修正しない）:**
> - 「誤指定の影響は最大 41.5 倍（Scen.C）／根拠 CSV が特定できていない・強さ=弱い」の行 →
>   **根拠 CSV は特定済み。** `expfam/results/exp_scenario_C_exp4_mismatch.csv` の
>   est X=Gaussian, Y=Poisson（両側誤指定）条件で **41.45×**
>   （`reports/mismatch_audit/mismatch_audit_report_20260708.md` §1、KI-003）。
> - 「実データへの適用は未実施」の行 → 実施済み（下記 2026-07-19 の更新注記のとおり）。

作成日：2026-05-31

> **（2026-07-19 更新注記）** 本表は 2026-05-31 時点の状態であり、以下の行は現状と異なる。
> - 「実データへの適用は未実施（本格的なもの）」→ **実施済み**。実データ実験フェーズ
>   （Wine/Cora/MovieLens、fixed 版使用、2026-06-17〜07-07、main マージ済み）が存在する。
>   総括は `reports/real_data_experiment_summary.md`、位置づけは root `CLAUDE.md` 参照。
> - Wine 実験（run_wine_dual.py 行）→ 旧 0.5 版は引き続き未評価だが、fixed 版での
>   Wine 評価は実施済み（KI-006 の 2026-06-18 更新参照）。
> 本表の他の行（人工データ実験・学会予稿の主張）は引き続き有効。
