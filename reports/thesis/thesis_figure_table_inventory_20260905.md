...
| 項目 | 内容 |
|---|---|
| 目的 | G1–G7 の乖離を行番号つきで示す（**うち G6・G7 は「問題なし」と判定済み**なので、実質的な乖離は G1–G5 の 5 件） |
...
> **⚠ Human Gate（root `CLAUDE.md` §6）:** 本章の証拠は **lineage E（objective-consistent
> experimental prototype）** であり、`EXPERIMENT_REGISTRY.md` は **`原稿採用 ✗`**、
> 結果レポート §9 限定 8 は **「本文採用不可」** と記録している。
> **prototype を manuscript evidence へ昇格させる判断は Human Gate であり、
> 本文書はその判断を代行していない。** 本章は人間の承認が得られた場合にのみ本文に入る。
> 承認が得られない場合は付録／予備的検討として扱う。
...
| 支える主張 | **QUALIFIED ONLY** |
| 限定 | **有限 4 点の記述。一致性ではない。** 反復は 8 のみ。S1・S2 の誤りはほぼ一貫して過小選択（**S3 は逆に過大選択**）。**一致は `K_TRUE` とのものであり `K*` とのものではない** |
| 章節 | 8.4 |
