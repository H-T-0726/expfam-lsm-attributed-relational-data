...
**以下の「真値一致」はすべて `K_TRUE`（generator の手続き上の設定）との一致であり、`K*`（観測分布の最小潜在次元）との一致ではない。** `K* ≤ K_TRUE` で等号は自明でなく、clean construction で `K* = K_TRUE` が成り立つかは未検証（U9）。
...
（§13 と同じく、真値一致は `K_TRUE` との一致であって `K*` との一致ではない。）
